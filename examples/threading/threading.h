...
struct thread_data{
     ...
     */
    pthread_mutex_t *mutex; //Locking the Mutex
    int wait_to_obtain_ms;  //time to wait 
    int wait_to_release_ms; //time after which mutex lock is released
    bool thread_complete_success; //status for completion	
    /**
